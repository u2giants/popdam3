-- Fix "function max(uuid) does not exist" in clear_style_group_batch.
-- The 20260407004832 migration rewrote the function to use MAX(id) on a uuid column,
-- but PostgreSQL does not have a built-in max() aggregate for the uuid type.
-- Replace with a subquery (ORDER BY id DESC LIMIT 1) which works correctly.
