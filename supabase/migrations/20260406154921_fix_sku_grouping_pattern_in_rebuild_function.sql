-- Fix rebuild_style_groups_batch: require purely alphanumeric SKU segments.
-- The old pattern '^[A-Za-z]{1,6}[0-9]' matched product-category folder names like
-- "AA1_VS1 - Canvas with foil", creating mega-groups of 1000+ unrelated assets.
-- The $ anchor restricts to purely alphanumeric names (no spaces/underscores/dashes).
-- Also changes ORDER BY to ASC (outermost match wins, consistent with JS extractSkuFolder).
  ...
  asset_skus AS (
    SELECT ab.*,
      (
        ...
        WHERE seg ~ '^[A-Za-z]{1,6}[0-9][A-Za-z0-9]*$'
          AND length(seg) >= 10
