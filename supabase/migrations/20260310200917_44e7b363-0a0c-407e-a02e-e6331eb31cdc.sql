...
BEGIN
  ...
  --    SKU pattern: first path segment matching ^[A-Za-z]{1,6}\d with length >= 10
  WITH asset_batch AS (
    SELECT a.id, a.relative_path, a.filename, a.file_type,
           ...
           a.workflow_status, a.created_at, a.modified_at
    FROM assets a
    ...
    SELECT ab.*,
      (
        ...
        WHERE seg ~ '^[A-Za-z]{1,6}[0-9]'
          AND length(seg) >= 10
          AND ord < array_length(string_to_array(ab.relative_path, '/'), 1)  -- exclude filename
        ORDER BY ord
        LIMIT 1
      ) AS sku
    FROM asset_batch ab
  ),
  -- Count total fetched for done detection
  batch_stats AS (
    SELECT count(*)::int AS total_fetched,
           max(id) AS last_id
    FROM asset_skus
  ),
  -- Assets with valid SKUs
  grouped_assets AS (
    SELECT * FROM asset_skus WHERE sku IS NOT NULL
  ),
  -- For each SKU, pick a representative asset (first by id) for group metadata
  ...
  -- Build folder_path from relative_path: everything up to and including the SKU segment
  sku_with_folder AS (
    SELECT sr.*,
      (
  ...
  SELECT
    (SELECT last_id FROM batch_stats),
    (SELECT count(*)::int FROM upserted_groups),
    (SELECT count(*)::int FROM assigned),
    (SELECT count(*)::int FROM asset_skus WHERE sku IS NULL),
    ((SELECT total_fetched FROM batch_stats) < p_batch_size)
  INTO v_last_id, v_groups_created, v_assets_assigned, v_ungrouped, done;
  ...
  RETURN QUERY SELECT
    ...
    v_assets_assigned,
    v_ungrouped,
    ((SELECT count(*)::int FROM (SELECT 1 FROM assets WHERE is_deleted = false AND (p_last_asset_id IS NULL OR id > p_last_asset_id) LIMIT p_batch_size) x) < p_batch_size);
END;
$$;