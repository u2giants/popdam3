...
|---|---|---|---|
| 1. Freeze baselines and confirm exact production sources | ✅ done | 2026-08-07 | Production Supabase baseline and authoritative DesignFlow Cloud SQL were queried read-only. Cloud SQL has 121 Depth rows, 19,671 Item Headers, and 661 MG04 rows. All 121 Depth lookup rows and every nonblank Depth usage total match the Supabase mirror; the 209 newer Cloud SQL Item Headers all have blank Depth. Evidence: shared-db `docs/verification/master-data-designflow-reference-cutover-20260807/cloudsql-depth-mg04-resolution.md`. |
...
| 3. Compare MG04 Sizes and build the guarded importer | 🟨 partial | 2026-08-07 | Direct ColdLion pagination is complete: 37/37 headers, `totalPages=1`; MG04 Size arrays total 530. Authoritative Cloud SQL has 661 legacy MG04 rows. All 17 gaps are resolved: 8 inactive historical Size rows with zero current code usage must remain nonselectable legacy refs; 9 EP001 rows mean Pages, not Size, and are excluded. The 11 normalized label differences retain the same division-plus-code identity; ColdLion is current label authority and legacy labels remain in history. `core.product_size` and its importer remain unbuilt. |
...
| 6. Compare and move `itemDepth` to `core.product_depth` | 🟨 partial | 2026-08-07 | Authoritative Cloud SQL snapshot captured through the dedicated read-only account: 121 lookup rows, SHA-256 `6a8dd274e380bbb12fd997d916858a75d5f6ae61872be02a92432f2a1f92c1ce`, with zero row differences from the Supabase mirror. All nonblank usage totals match; `0.63` is present. Comparison is complete, but `core.product_depth`, source refs, audit, protected functions, and DB Data Admin UI remain unbuilt. |
...
Fresh-session instruction: Natural Context Cut Point A was reached on 2026-08-07, and the Cloud SQL/MG04 evidence gaps were closed later that day. Record Albert's three Release 1 owner decisions from the handoff before Step 8: Product Depth maintainers, inactive-MG04 new-selection policy, and PopDAM's linked-item write path. Phase 2 ambiguous Packaging/Designer/Vendor mappings may remain deferred. Before each new phase, re-read this entire plan and update this STATUS table with exact commits, PRs, verification results, and blockers.
