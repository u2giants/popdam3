...
|---|---|---|---|
| 1. Freeze baselines and confirm exact production sources | 🟨 partial | 2026-08-07 | Read-only production Supabase baseline captured at 2026-08-07T22:34:56Z: `core.packaging_type` 19, `core.product_size` absent, `core.creative_designer` 22, `core.factory` 93, `dflow.itemDepth` 121, `dflow.merchGroup` 3,645, and `dflow.itemHeader` 19,462. Current DesignFlow `develop` SHAs for all six repos are recorded. Production Cloud SQL authority is confirmed by configuration, but direct row equality remains blocked because the Supabase `itemDepth` mirror's newest Airbyte timestamp is 2021-08-19. Evidence: shared-db comparison branch `codex/reference-cutover-comparisons`, `docs/verification/master-data-designflow-reference-cutover-20260807/`. |
| 2. Compare Packaging Types | ✅ done | 2026-08-07 | All 8 hard-coded values, all 19 `core.packaging_type` rows, and all Item Header values are classified: 1 alias candidate, 7 DesignFlow-only, 19 core-only; live usage is 19,461 blank and 1 `Cardboard corners`. Phase 2 only. Evidence: `comparison-summary.json` and `comparison-report.md` in the dated shared-db verification folder. |
| 3. Compare MG04 Sizes and build the guarded importer | 🟨 partial | 2026-08-07 | Direct ColdLion pagination proven: `/merchGroupHeaders` page 0 returned 37/37, `totalPages=1`; MG04 details are plain arrays totaling 530 (SP001 187, EH001 156, CW001 187), each hashed. The DesignFlow mirror has 661 rows: 644 mirrored rows match a direct composite identity, 17 are mirror-only, and 11 matched rows have label conflicts; EP001 is absent from the direct header feed. Production `core.product_size` and a matching migration-ledger entry are both absent. Importer creation is blocked until the 17 missing identities and label conflicts are explained. |
| 4. Compare DesignFlow item assignments with `core.creative_designer` | ✅ done | 2026-08-07 | Five assignments classified: 3 `creative_designer` assignments across 3 items all point to one active DesignFlow user with no exact canonical designer match; 2 `creative_director` assignments are excluded. Phase 2 requires one approved person mapping. Evidence: dated comparison summary. |
| 5. Compare DesignFlow Vendors with ColdLion-backed `core.factory` | ✅ done | 2026-08-07 | Existing importer dry-run pulled 97 ColdLion Vendors and rolled back; it proposed 0 inserts, 95 updates, 1 failure, and 1 skip. Mirror has 169 legacy Factory rows versus 93 core rows; 14 exact normalized names and 155 Phase 2 review candidates. Mirrored Item Headers have zero nonblank Vendor references. Evidence: `vendor-dry-run.json` and dated comparison summary. |
| 6. Compare and move `itemDepth` to `core.product_depth` | 🟨 partial | 2026-08-07 | Read-only mirror comparison accounts for 121 active lookup rows and all 98 distinct Item Header text values; 49 nonblank values are unmatched after numeric/unit normalization and 5 are ambiguous. `0.63` exists in the mirror. The mirror cannot prove current production Cloud SQL because its newest Airbyte timestamp is 2021-08-19. No schema or UI work started. Import is blocked pending an authoritative Cloud SQL snapshot. |
...
Fresh-session instruction: Natural Context Cut Point A was reached on 2026-08-07 after all six read-only comparisons. Resume by closing the Step 1/3/6 evidence gaps before Step 7: obtain an authoritative current Cloud SQL `itemDepth` snapshot, explain the 17 MG04 mirror-only identities and 11 label conflicts, and then regenerate the reports. Do not begin schema or importer work while these totals remain unbalanced. Before each new phase, re-read this entire plan and update this STATUS table with exact commits, PRs, verification results, and blockers.
