...
async function thumbnailPsd(filePath: string): Promise<ThumbnailResult> {
  try {
    const psdWork = async () => {
      // Pre-read into buffer so the OS file handle is released before we start
      // the sharp pipeline.  If the pipeline hangs and we time out, the file
      // descriptor is already closed — no leaked handles.
