...
| 3. Add authorization-safe hybrid search contract | 🟨 routed | Shared-db orchestrator + PopDAM edge function | Remaining ranked authorization/pagination/facet contract routed as `u2giants/shared-db#1703`, dependent on effective-filter contract #1645; edge now uses the caller client instead of service-role search | Await governed preview/production contract and cross-user authorization proof before hybrid activation |
| 4. Add resumable embedding worker operation | 🟨 implemented, dark | PopDAM `main` | `apps/worker/src/handlers/embed-search.ts`, lease-only edge action, operation registration/tests | 134 worker tests and worker build pass locally; deployment plus Railway restart/resume proof pending |
...
| 6. Add admin coverage and start/stop UI | 🟨 implemented | PopDAM `main` | `SearchIndexCard` with coverage, start/stop/resume, and terminal requeue | Frontend build passes; component tests and desktop/mobile visual proof pending |
