...
- **Frontend deploy auth fixed** (2026-06-10): production was still on commit `8c0508d` because every later `Publish Frontend Image` run passed build but failed at "Log in to GHCR", so no newer `popdam-frontend:latest` image reached GHCR and Coolify stayed on the old container. `.github/workflows/publish-frontend.yml` now uses the workflow `GITHUB_TOKEN` (`packages: write`) instead of `secrets.GHCR_PAT`, and has `workflow_dispatch` for manual redeploys. Future stale-site checks: compare the live header SHA to the latest successful `Publish Frontend Image` run first.
