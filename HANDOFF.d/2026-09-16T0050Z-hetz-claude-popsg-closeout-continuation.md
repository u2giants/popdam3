...
1. **Turn the Windows render agent back on.** It is the machine that makes PopSG previews and extracts PDF text. It last reported in on 2026-09-14 and has been offline since. Nothing in the PDF part of this project can move until it is running again — not the two leftover PopDAM files, and not the 4,318 PopSG PDFs that have never been processed at all. *Recommendation: power it on and confirm it shows "Live" in PopSG Settings → Render Agent.* Blocks plan step 3 entirely.

2. **Approve a supported administrator path to the style-guide NAS (`edgesynology2`), or do the one-click refresh yourself.** The file-scanning bridge is running version 1.16.11; 1.16.12 has been published since 2026-09-11 and cannot install itself because the running container cannot see its own definition file. The fix is a single Container Manager project refresh on that NAS. The previous session's helper account did not have the access to do it and correctly stopped rather than forcing it. *Recommendation: do the project refresh yourself, or grant a protected admin path so a session can.* Blocks plan step 6.

### A wrong guess is recoverable, but confirm

3. **The style-guide NAS password is displayed in plain text on the PopSG Settings page to anyone with the admin role.** This is the known, already-fixed-at-the-database-level exposure from 2026-07-24 — admins are *meant* to see it, non-admins no longer can. But the value is still one of the eight credentials flagged for rotation back in July, and rotation has never happened. It is an Active Directory account, so rotating it is an AD change. *Recommendation: decide whether to rotate now or formally accept the risk and close the item — it has been open for seven weeks.* Does not block any step here.
...
- **Plan step 3, PDF text extraction, is blocked only by the offline Windows agent.** Both PDF queues are otherwise ready. See §5 for why the "remainder of 2" the predecessor treated as a defect is not one.
- **Plan step 6 in the predecessor's numbering, bridge 1.16.12 adoption, is unchanged.** `synology-bridge-1` heartbeats every 30 seconds on version `1.16.11`, build `6ca879e7e23d6e0eacec55f9e8e460afbbb4fca0`. The read-only Compose mount added in `fc7bfa91` still needs one Container Manager project refresh to take effect.
...
5. **Screenshots of the PopSG Settings page were deliberately NOT taken or saved.** That page renders the NAS password in plain text and the render-error list contains licensed file paths. No screenshot, path, filename, cookie or credential was written to any file, commit or issue.
...
5. Once the owner confirms the Windows render agent is back online and it shows "Live" in PopSG Settings → Render Agent, resume the existing PopDAM PDF job through the normal authenticated admin route — it will not restart itself because its stored status is `completed`. Wait for its terminal authoritative remaining count. **You'll know it worked when:** `count_pdf_backfill_remaining()` returns 0 and the job reports completed with no new failures.
...
7. For bridge `1.16.12`, once the owner supplies or performs the supported administrator path, refresh the Container Manager project so the read-only Compose mount exists, then invoke the normal update. **You'll know it worked when:** the bridge heartbeat reports `1.16.12`, the container is still Compose-managed, restart count has not grown, heartbeats are continuous, rollback is preserved, and no NAS content changed.
...
- No NAS write, bridge update, shared-db migration, preview environment, production promotion, or database write was performed in this session.
...
- **Two of the three blockers need physical access nobody in a session has.** If the Windows machine and the NAS refresh both stay unavailable, #107 cannot close no matter how the database lane goes, and that should be said plainly rather than worked around.
