...
- **Bridge 1.16.12 is ADOPTED and verified.** *(Updated 2026-09-16 11:00Z.)* Version `1.16.12`, build `edcd8341bb8e3b054a3ff55601f4c61aeb4f39d9`, running, restart count 0, Compose ownership intact (`com.docker.compose.project.working_dir=/volume1/docker/popdam`), same agent id with no re-pair or duplicate registration, continuous heartbeats, no `last_error`, healthy NAS mount, rollback retained, no NAS content written. Evidence: `verification/popsg-readiness/bridge-1.16.12-adoption-2026-09-16.md`. A benign startup warning notes Node 20.20.2 versus the AWS SDK's Node 22 requirement for releases after early 2027 — worth scheduling, not urgent.

- **The ordinary nightly crawl is now failing, and the three-clean-night proof has reset.** *(New 2026-09-16 11:00Z.)* The 2026-09-15 22:00 run found and accepted 217,047 files with 0 deactivated, `stale_remaining=0` and `guard_state=ok`, then failed with `Matview refresh failed: canceling statement due to statement timeout`. It is recorded `status=failed`, `lifecycle_state=refreshing`, `refresh_completed_at` null, so `style_guide_file_groups` and `style_guide_folders` are stale and the Guides grid and folder tree read from them. Filed as shared-db #3023. It will recur nightly until repaired.

<details><summary>Superseded: the pre-adoption bridge diagnosis, kept because it explains the fix</summary>

- **Bridge 1.16.12 adoption was diagnosed and staged, and needed one `sudo` run.** *(Updated 2026-09-16 01:35Z.)* `synology-bridge-1` heartbeats every 30 seconds on `1.16.11`, build `6ca879e7e23d6e0eacec55f9e8e460afbbb4fca0`. The live bridge is on **`edgesynology1`**, not edge2 — edge2 has a stale `/volume1/docker/popdam` folder but the running, data-volume-backed, rollback-stamped deployment is on edge1. Its on-NAS `docker-compose.yml` is missing both the `POPDAM_COMPOSE_PATH` environment entry and the `/volume1/docker/popdam:/app/compose:ro` mount that repo commit `fc7bfa91` added, which is exactly why the self-updater stops safely. A re-runnable script that takes a timestamped backup, patches the compose file in place preserving edge1's own host paths, records the current image id for rollback, pulls the published image, recreates the Compose-owned service and prints the resulting state is staged at `/volume1/docker/popdam/apply-bridge-1.16.12.sh` on edge1. **The NAS itself was not modified** — only that script was placed. It needed `sudo`, which the owner supplied.</details>
...
- **Three different functions now fail or nearly fail against the same 8 s `authenticated` ceiling**, which is why they are best read as one problem: `search_style_guide_library_v2` on the unfiltered default browse (#3009), `claim_pdf_backfill_batch` on every call (#3009), and `refresh_style_guide_matviews` every night (#3023). The corpus crossed 217,000 files this week and things that used to just fit no longer do. Expect more of the same as it grows; a repair that only buys a second or two will not hold.
...
7. Bridge `1.16.12` is done — do not repeat it. Instead, after shared-db #3023 is repaired, confirm the nightly crawl recovers on its own. **You'll know it worked when:** three consecutive ordinary nightly runs record `status=completed`, `guard_state=ok`, `stale_remaining=0` and a non-null `refresh_completed_at`, with no manual crawl among them.
...
- **NAS container work follows an open-then-close ritual, by the owner's standing instruction (2026-09-16).** At the START of a block of container work, ask him once to run `sudo sh /volume1/docker/popdam/ai-docker-sudo-enable.sh` on the NAS you need. Do the whole block. At the END, run `sh /volume1/docker/popdam/ai-docker-sudo-disable.sh` yourself — it needs no password and proves the access is gone. Never leave the grant open past the block, and never ask him to enable it for a single command. Both scripts are checked in at `deploy/synology/` so the NAS is not hidden state. Be honest about what the grant is: Docker socket access is root-equivalent on these boxes, which is why the earlier standing grant was disabled on 2026-09-03. Minor hardening note for later: `/volume1/docker/popdam` is mode 777 on both hosts, so the scripts a root user runs from there are writable by any account with share access.
...
- **The remaining blockers are all in the database lane now.** Three issues, one ceiling: #3009 and #3023.
- **The growth trend is the underlying risk.** Three functions crossed the limit in one week as the corpus passed 217,000 files. Treat a repair that lands just inside the ceiling as a deferral, not a fix.
- **Superseded:** The Windows machine is back. If shared-db #3009 stalls, #107 cannot close no matter what is done on the app side, and that should be said plainly rather than worked around.
