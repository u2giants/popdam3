...
| 7. Combine scopes in search and UI | 🟨 in progress | 2026-08-30 | **Application half complete and GLM-approved** (scoped detail panels, `useEffectiveAssetTags`, normalized scoped write contract with admin-gated group edits and review decisions, docs, synthetic visual evidence in `verification/ai-tagging-scope/2026-08-27T0321Z/ui/`). **Filtering half BLOCKED on a production regression.** Shared-db [#1645](https://github.com/u2giants/shared-db/issues/1645) shipped `20260830110517_popdam_effective_asset_filters.sql` to production (`filter_effective_assets`, `get_effective_filter_counts`, `asset_effective_tags`, and a delegating `get_filter_counts`), but acceptance criterion 6 — the cold 8s `authenticated` ceiling — was never measured and **is not met**. Measured against production with a real `authenticated` JWT on 2026-08-30: `filter_effective_assets` returns 500 / `57014 statement timeout` at ~8.1s for every payload including an EMPTY one, and `get_filter_counts` now 500s whenever `licensorId`/`propertyId`/`tagFilter` is present — a path reachable from the live filter sidebar today with no app change. Legacy no-filter counts still return but at 6.31s against the 8s ceiling. Filed as [shared-db#1945](https://github.com/u2giants/shared-db/issues/1945) with the likely cause: `filter_effective_assets` carries `set search_path`, and a SQL function with a `SET` clause cannot be inlined, so PostgREST's LIMIT never pushes down and the full 135k-row join materialises per call. The PopDAM wiring is written and tested but **gated off** behind `EFFECTIVE_SCOPE_CONTRACT_READY = false` in `src/hooks/useAssets.ts`; turn it on only after #1945 is fixed and re-measured cold as `authenticated` against production. |
