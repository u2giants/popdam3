...
| 7. Combine scopes in search and UI | 🟨 in progress | 2026-08-30 | **Application half complete and GLM-approved** (scoped detail panels, `useEffectiveAssetTags`, normalized scoped write contract with admin-gated group edits and review decisions, docs, synthetic visual evidence in `verification/ai-tagging-scope/2026-08-27T0321Z/ui/`). **Filtering half still BLOCKED — the contract regressed again.** Shared-db [#2054](https://github.com/u2giants/shared-db/issues/2054) (migration `20260902042548`) fixed its own target: `get_filter_counts` with a licensor filter is now **8 of 8 passing at 0.11–0.43s**, up from 4 of 8 failing. But re-measured against production with a real `authenticated` JWT on 2026-09-02, the contract as a whole is now **worse than after #1945**: `filter_effective_assets` times out on every call — 0 of 3 passing at ~8.1s for five rows, one column, no filter and no count header, where it was 0.94–0.99s — and the tag facet count went from 7 of 8 passing to **0 of 8** across four different tags. `count=exact` still never completes. Traced to the `with authorized as materialized (select public.require_dam_access() ok)` entitlement CTE introduced by `20260901130428` / `20260901142825` (PopDAM #97 ranked-search work, not #2054): a `MATERIALIZED` CTE is an optimization barrier, so the function can no longer be inlined and PostgREST's LIMIT cannot push down — the same failure class as the original `set search_path`, reintroduced by a different route. Filed as [shared-db#2138](https://github.com/u2giants/shared-db/issues/2138). PopDAM wiring stays **gated off** behind `EFFECTIVE_SCOPE_CONTRACT_READY = false` in `src/hooks/useAssets.ts`; before enabling, measure rows, the tag count, and a usable list total over several consecutive calls each — a single warm call has passed at every stage of this while the real shapes failed. |
