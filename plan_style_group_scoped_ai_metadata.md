...
| 7. Combine scopes in search and UI | 🟨 in progress | 2026-08-30 | **Application half complete and GLM-approved** (scoped detail panels, `useEffectiveAssetTags`, normalized scoped write contract with admin-gated group edits and review decisions, docs, synthetic visual evidence in `verification/ai-tagging-scope/2026-08-27T0321Z/ui/`). **Filtering half still BLOCKED — counting, no longer row fetching.** Shared-db [#1645](https://github.com/u2giants/shared-db/issues/1645) shipped the contract to production and [#1945](https://github.com/u2giants/shared-db/issues/1945) (migration `20260831044913`) fixed the original timeout: `filter_effective_assets` now returns 200 rows in **0.99s** (was 500 at 8.15s) and the legacy no-filter `get_filter_counts` path is **0.54s** warm. Re-measured against production with a real `authenticated` JWT on 2026-08-31, two problems remain: `count=exact` over `filter_effective_assets` **never** completes (500 `57014` at 8.07s for both 25 and 200 rows, while the same calls without a count are ~0.95s), which is the shape the list needs for pagination; and `get_filter_counts` with an effective filter **flaps on the ceiling** — 8 consecutive licensor calls gave 4 passes and 4 failures (2.0s–8.1s), so the live sidebar regression is narrowed, not closed. Filed as [shared-db#2054](https://github.com/u2giants/shared-db/issues/2054), with the pointer that adding a filter makes counting slower (`{}` 3.26s vs `{licensorId}` 8.07s) because the identity predicate is a `CASE` across `assets` and `style_groups` and cannot use an index on either. PopDAM wiring stays **gated off** behind `EFFECTIVE_SCOPE_CONTRACT_READY = false` in `src/hooks/useAssets.ts`; enable only once a list total is obtainable inside the ceiling and the facet count passes over several consecutive calls, not one. |
