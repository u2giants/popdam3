...
| 7. Combine scopes in search and UI | 🟨 in progress | 2026-08-27 | **Application half complete and GLM-approved.** `src/hooks/useEffectiveAssetTags.ts` reads the governed `get_effective_asset_metadata`; `src/components/library/ScopedTagSections.tsx` renders separate **Style Group** and **This file** sections with source labels, candidates, rejected tombstones and restore; both detail panels now write through one normalized contract (`add-scoped-tag`, `remove-scoped-tag`, `review-scoped-tag` in admin-api) instead of mutating `assets.tags` or `asset_tags` directly. Manual edits default to This file; group edits and every review decision require an admin role, server-enforced and mirrored in the UI. Master Data facts can be neither rejected nor hand-overwritten. `docs/UI_OVERVIEW.md` updated; synthetic visual evidence in `verification/ai-tagging-scope/2026-08-27T0321Z/ui/`. **Still pending: effective-tag filtering and grouped-identity filtering through the indexed path, with facet-count parity and the cold 8s gate.** Today `useAssets.ts` filters `assets.tags @>` and the asset identity columns, which cannot express the effective scope — group tags are deliberately absent from `assets.tags`. That needs a governed shared-db contract, now requested as [`u2giants/shared-db#1645`](https://github.com/u2giants/shared-db/issues/1645) (`db-work`), carrying the cold 8s `authenticated` ceiling, the `idx_assets_facet_counts` covering-index constraint, candidate/rejected exclusion, seven acceptance criteria, and two design options. It is queued on the `db-work` board: the active orchestrator marker (shared-db#1632) declares a codex session on EDGE-DEV that this session cannot message directly, so it was queued rather than dispatched, with a routing pointer left on #1632. PopDAM takes the app-side wiring, generated types, and the parity/performance tests once the contract lands. |
