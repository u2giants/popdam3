...
| 3. Make crawl completion truthful and restart-safe | ✅ complete | Three consecutive ordinary September 11–13 crawls each found/upserted 216,702 files, passed the guard, reconciled to zero stale rows, stamped fresh aggregates, and completed; first run safely inactivated the proven 11,618 ghosts, later runs changed zero rows |
...
| 5. Add crawl regression protection and operator visibility | 🟨 in progress | Truthful production Admin health panel deployed in `4514cc43`; signed-in attention-required desktop/mobile proof in `verification/popsg-readiness/crawl-health-qa-2026-09-07.md`; healthy/reconciling production proof still pending |
...
| 7. Finish searchable PDF/source coverage | 🟨 in progress | Shared-db #2507/v2 terminal outcome contract and app/Windows wiring are in production. PopDAM authoritative count is 2, but claim still times out; false completion fixed in `825d7aca`. PopSG execution remains serialized behind governed #2792 |
| 8. Build comprehensive PopSG search and filters | 🟨 in progress | Shared-db #2506 is closed and its v2 objects are reported in production, but a natural default Files call still hits SQLSTATE 57014 at the normal ceiling. Successor #2860 is re-routed to sole active marker #2925; app cutover/final QA remain pending |
