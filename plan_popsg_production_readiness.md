...
| 2. Route and ship the shared-database contract | ✅ complete | `u2giants/shared-db#2212`; PR #2339 merged as `9e83aa04`; migration `20260905104802` preview-tested and verified in Virginia production; generated consumer types synced in `fc0513fc` |
| 3. Make crawl completion truthful and restart-safe | 🟨 regressed 2026-09-15 | Sept 11–14 ordinary crawls completed cleanly. Sept 15, 16 and 17 crawls ingested and reconciled cleanly (guard ok, 0 stale) but failed at `refresh_style_guide_matviews` (statement timeout). Fix shipped as shared-db #3023 (closed 2026-09-18 17:50Z, queue-driven refresh, 1.99 s candidate query); its end-to-end proof is the next ordinary nightly crawl. Three fresh clean nights required again |
| 4. Reconcile the current ghost rows safely | ✅ complete | `verification/popsg-readiness/baseline-2026-09-14T022122Z.json`; same-window edge2 scan exit 0, 216,702 eligible, zero inaccessible, exact path-set parity with 216,702 active Virginia rows, zero missing/extra |
| 5. Add crawl regression protection and operator visibility | ✅ complete 2026-09-16 | Administrator-console acceptance on build `f341443`: all four Settings tabs, zero 4xx/5xx, zero console errors, healthy/reconciling/completed/failed states observed — `verification/popsg-readiness/admin-console-qa-2026-09-15.md` |
| 6. Finish and classify preview coverage | ✅ complete | Shared-db #2509 is in production; fresh service-role stats complete in 605 ms and classify all 216,702 active rows as 214,694 previewed plus 2,008 render exceptions with zero unexplained. Protected signed-in production QA renders the full 216,702 Preview Coverage total (`signed-in-qa-2026-09-14.json`) |
| 7. Finish searchable PDF/source coverage | 🟨 blocked | Windows agent online and healthy. `claim_pdf_backfill_batch(10)` still hits SQLSTATE 57014 at 8 s (re-measured 2026-09-18); it fell between #2792 and #3009, re-filed as shared-db #3282 under marker #3275. PopDAM job `paused` with 2 remaining; PopSG 0 of 4,318, serialized behind it |
| 8. Build comprehensive PopSG search and filters | 🟨 unblocked — app cutover next | Shared-db #3009 repaired the default browse (snapshot, cron */5); re-measured 2026-09-18 at 17–223 ms both modes. Remaining: app cutover from direct reads in `PopSGLibraryPage.tsx:617-705` to `search_style_guide_library_v2`, deploy, signed-in QA |
...
**Where to start (2026-09-18):** step 8 app cutover is unblocked and is the next app work; step 7 waits on shared-db #3282; step 3 needs three clean ordinary nights after #3023. Bridge `1.16.12` is adopted (see drift 2026-09-16 11:00Z) — do not redo.

**Fresh-session start:** begin with this STATUS table; do not repeat a completed baseline, crawl proof, preview proof, or bridge incident. Resume at the earliest in-progress/open gate, then re-read all downstream phases and update this table after each accepted phase; at each marked context cut, use the `fresh-session` skill.
...
**Incident prevention — 2026-09-03 18:05Z:** v1.16.11 removes both automatic topology mutations that amplified the incident: self-update no longer falls back to an unmanaged `docker run`, and a read-only mount is reported rather than silently recreated as writable. The manual updater no longer runs `compose down`; it validates configuration, recreates only the bridge, waits 45 seconds, verifies zero restarts plus Compose ownership, and rolls back to the prior image on failure. The reference Compose removes unsupported Synology NanoCPU controls and makes only the PopDAM mount writable; Style Guides remains read-only. Four regression tests lock these conditions plus the clean-image eligibility import. Production build `6ca879e7e23d6e0eacec55f9e8e460afbbb4fca0` passed CI and clean image publication, then remained Compose-managed with restart count zero across the deployment window and consecutive heartbeats.

**Edge2 recovery — 2026-09-03 18:16Z:** Outlook evidence showed the continuing alerts came from `edgesynology2`, not a NAS reboot; host uptime remained 15 days. Its detached `popdam-bridge` still ran the first broken JSON-loader image and had reached 75 restarts. The container was stopped and preserved for rollback, stable was pulled, and only the bridge service was recreated from `/volume1/docker/popdam/docker-compose.yml`. Production acceptance showed v1.16.11 / `6ca879e7e23d6e0eacec55f9e8e460afbbb4fca0`, Compose project `popdam`, restart count zero, successful cloud registration, and consecutive heartbeats; alert mail stopped before the new container start. Edge2's Style Guides/other NAS mounts remained read-only and no NAS or database records were changed. Temporary passwordless Docker sudo was removed from both NAS hosts after verification.

**Execution drift — 2026-09-07 12:14Z:** shared-db #2212 is merged and migration `20260905104802` is present in the confirmed Virginia production ledger. PopDAM PR #109 deployed the new RPC order, but the ordinary 2026-09-05 and 2026-09-06 crawls exposed an application-contract omission: both found 216,374 files while the run row still held its default `files_found=0` when the database guard evaluated it. The guard correctly set `attention_required`, reported `empty_crawl`, and inactivated zero rows. Commit `24013e55` now persists discovered and accepted counts before reconciliation and returns at most one 5,000-row batch per Edge request using the already-deployed bridge continuation contract. All 306 app tests, bridge tests/build, lint with zero errors, and production build passed; CI and Edge deployment succeeded, and Virginia `agent-api` is v15. No manual crawl was used as acceptance. Phases C–E were re-read; the next gate remains the first clean ordinary nightly run, followed by same-window private reconciliation evidence.

**Execution drift — 2026-09-07 12:27Z:** pre-run review of the deployed search-refresh contract found that one completion request synchronized only the first bounded 5,000-document slice, while the app would immediately mark the crawl complete. Commit `9836aa9e` now treats a full slice as continuation evidence, clears the premature per-call freshness stamp, and returns additive `state=refreshing` until a short final slice proves the run corpus is fully synchronized. Focused lifecycle tests pass and the Edge deployment succeeded before the next ordinary crawl. This does not start a crawl, backfill, or parallel workload. Phases C–E were re-read; the first clean ordinary nightly run remains the next production gate.

**Execution drift — 2026-09-07 12:38Z:** current Virginia aggregates show 227,992 active library rows and 227,992 active v1 search documents, but the deployed v1 RPC cannot paginate guide groups or supply the approved preview, render-exception, and content-availability filters. Structural successor #2506 was queued into the already-active shared-db orchestrator marker #2497; no competing orchestrator or structural edit was started. PDF capacity also remains incomplete: 23,549 actionable PopDAM PDFs, 4,436 active PopSG PDFs with zero dedicated extraction rows, 734 resolved and 87 unresolved source rows. Windows maintenance serialization shipped in `e799c054`; its first release attempt exposed a missing packaged test dependency, repaired in `11a90404`. Release `0.16.3.158` published successfully; no PDF workload was started before production agent adoption. Phases D–E were re-read; reconciliation proof, serialized PopDAM completion, then PopSG extraction/search remain the safe order.

**Execution drift — 2026-09-07 13:00Z:** the live Windows agent adopted `0.16.3.159`, and the single PopDAM PDF backlog was started through the normal authenticated Admin route only after live render leases were zero. The first attempt exposed false progress: 350 results were reported processed while zero rows landed because the Edge handler ignored its sample-upsert result. The run was paused without deleting data. Commit `36f879b6` now fails the batch on any write error or partial insert and increments progress only from returned inserted IDs. After Edge deployment and controlled resume, 50 durable rows landed and the authoritative remaining count fell from 23,549 to 23,499. PopSG extraction endpoints and the serialized Windows runner are deployed but remain untriggered. A second #2212 contract omission—`skipped` exists but the completion RPC cannot record terminal skips or extraction method—was routed as shared-db #2507 under active marker #2497. Phases D–E were re-read; only the PopDAM backlog is running.

**Execution drift — 2026-09-07 13:14Z:** signed-in desktop/mobile QA accepted the crawl-health panel itself, but broader Settings QA found repeated HTTP 500 responses from `get_sg_preview_stats()`; Virginia logs confirmed normal statement-timeout cancellations. The timeout was not raised. Aggregate-only read evidence independently classified 220,290 active files with previews, 7,701 with stable coarse error categories, and one no-status row. Structural performance/category repair is queued as shared-db #2509 under existing orchestrator marker #2497. The QA artifact was corrected immediately and does not claim a clean console. Phases D–E were re-read; preview acceptance remains open.

**Execution drift — 2026-09-07 13:22Z:** preliminary signed-in production Library QA verified all 227,992 active files, 7,702 no-preview results, 4,436 PDFs, a 3,208-result representative franchise query, database-side pagination, and guide-detail opening. The detail sheet initially emitted a missing accessible-description warning; commit `8ec90b45` added hidden descriptions to both file and guide sheets, passed targeted lint/build and frontend CI, deployed through Coolify, and was rechecked in a fresh authenticated tab with zero console warnings/errors. This is not final v2 search acceptance: shared-db #2506 and PopSG PDF extraction remain gates. Phases C–E were re-read; their order is unchanged.

**Execution drift — 2026-09-07 13:35Z:** the deployed PopSG extraction endpoint had no operator surface. Commit `16cda85a` adds a separate PDF Search Coverage card without exposing a trigger, passed targeted lint/build and frontend CI, deployed through Coolify, and was verified signed-in with 4,436 active PDFs and zero extracted/waiting/failed/skipped rows. Its copy truthfully states that execution is gated on the PopDAM queue and governed terminal-outcome contract. The PopDAM workload remained the only active PDF loop and continued with zero recorded failures. Phases C–E were re-read; #2507 and serialized completion remain required.

**Execution drift — 2026-09-07 14:05Z:** live row acceptance caught false PopDAM PDF completion that the progress display had hidden: 3,436 readable PDFs landed as errorless failures after MuPDF document memory accumulated, while 1,102 rows had genuine missing-source errors. The run was paused; a targeted rerun extracted 1,876 characters from a false failure. Release `0.16.3.160` now explicitly releases MuPDF objects and retains stage errors. The 3,436 rows were exported to a protected rollback artifact, removed for retry, and counters reconciled. A 25-row write then hit the normal statement ceiling, so deployed Edge commit `0aa578b9` reduced the batch to three without raising timeouts. Live acceptance committed three consecutive batches: 9/9 searchable extracts, 942–2,971 characters, zero failures. The single PopDAM loop resumed with 22,398 actionable files at the reconciled checkpoint; PopSG extraction and crawls remain idle.

**Execution drift — 2026-09-11 17:12Z:** current Virginia reads found no qualifying three-night proof. Ordinary September 7–9 crawls each accepted 216,374–216,411 files, then truthfully failed when `reconcile_stale_sg_files_batch` exceeded the normal statement ceiling; September 10 stopped safely when its first 500-file ingest batch timed out. All four runs deactivated zero rows. Structural successor #2792 is queued under current marker #2758; no competing orchestrator or schema edit was started. The app repair reduces ingest writes to 100 rows and reconciliation updates to 250 rows without changing timeouts. Shared-db #2507 is live, and the Windows/Edge completion path now records explicit extracted/failed/skipped outcomes, method, and bounded terminal reason through the v2 RPC. Bridge tests, Windows tests/build, 370 repository tests, targeted lint, and the production build pass. PopDAM reports 382 actionable files remaining; PopSG extraction remains unstarted. Phases C–E were re-read; a clean ordinary run must restart the three-night gate.

**Execution drift — 2026-09-11 17:24Z:** PopDAM commit `edcd8341` is on `main`; CI, Edge deployment, bridge image `1.16.12`, and Windows release `0.16.3.161` succeeded. The Windows agent adopted the release and is healthy with healthy NAS access; the bridge pointer is live but its heartbeat still reports `1.16.11`, so ordinary self-update adoption remains pending. Fresh production probes showed both `claim_pdf_backfill_batch(1)` and `count_pdf_backfill_remaining()` terminate with SQLSTATE `57014` at the normal statement ceiling. This explains the stalled 382-row PopDAM remainder and blocks serialized PopSG extraction. The exact bounded/index-supported repair was added to governed successor #2792 under marker #2758; no timeout was raised and no competing database authoring began. Phases C–E were re-read: the order remains database repair, PopDAM drain, PopSG terminal classification, then ordinary-night and signed-in acceptance.

**Execution drift — 2026-09-11 17:38Z:** the normal authenticated bridge-update action initially rejected its documented service-role caller because `system` was written to a UUID audit column. Commits `afda7c53` and `c112cb82` repair and deploy that path; the action now succeeds. A production check proved bridge image `1.16.12` is available, but apply stopped recoverably because the running container cannot see its Compose file. Commit `fc7bfa91` adds the read-only project-definition mount and explicit Compose path, with a deployment-safety regression test. One Container Manager project update is required to establish that mount before self-update can adopt `1.16.12`; the current `1.16.11` container remains alive. No crawl/backfill was started. Phases C–E were re-read and their gates remain unchanged.

**Execution drift — 2026-09-13 22:22Z:** three consecutive ordinary crawls on September 11–13 completed with `guard_state=ok`, accepted reconciliation, `stale_remaining=0`, and fresh aggregate timestamps. The first safely inactivated the previously exported 11,618 ghosts; the next two changed zero rows. A low-priority edge2 exact-contract inventory completed in 13 seconds with exit 0, 216,702 eligible files, and zero inaccessible paths. The first local comparison exposed nondeterministic database paging in the evidence collector and was discarded; deterministic ID paging then proved exact 216,702-to-216,702 path-set parity with zero missing or extra rows in `baseline-2026-09-14T022122Z.json`. Raw evidence is mode 0600 under `.private/popsg-readiness/`. Production preview stats now finish in 605 ms with 214,694 previews, 2,008 classified exceptions, and zero unexplained. Crawl rows retain a display-only `lifecycle_state=refreshing` after their fully stamped completion; the completion writer now sets both status and lifecycle to completed. PopDAM reports completed despite an authoritative remainder of 2, so commit `825d7aca` removes the false processed-total completion shortcut. `claim_pdf_backfill_batch(1)` and the default v2 Files search still time out normally; #2792 was re-routed and fresh search successor #2860 opened under current marker #2858. Phases D–E were re-read; PopDAM drain, PopSG extraction, v2 app cutover, signed-in QA, and closure remain gated.

**Execution drift — 2026-09-13 22:55Z:** owner confirmation records shared-db #2506 and predecessor marker #2497 as closed and in production. Fresh issue checks still show performance successors #2792 and #2860 open and routed to the sole current marker #2858; no competing orchestrator or structural edit was started. Protected signed-in production QA successfully authenticated, loaded the Library, exposed Guides/Files switching and search, loaded Settings, and rendered the 216,702-file Preview Coverage total. This closes preview phase 6. The test account received four aggregate Admin API 403 resource errors, so final administrator-console acceptance is not claimed; the public evidence records only classifications and contains no licensed names or paths. Phases D–E were re-read: PDF drain remains ahead of PopSG extraction, and the v2 repair remains ahead of app cutover and final browser acceptance.

**Execution drift — 2026-09-15 00:26Z:** the PopDAM mirror advanced to shared-db `9bbd288`; shared-db #2506 has a closed read-only production-live-proof follow-up (#2921/#2922), consistent with the owner’s confirmation that #2497 and #2506 are production-complete. The live timeout successors #2792 and #2860 remain open, while referenced marker #2858 and successor marker #2893 are both closed. Both issues now record the routing drift and await re-admission by the next sole active orchestrator; this session did not create one or author any structural change. Phases D–E were re-read: the prerequisite order remains PopDAM PDF repair/drain, PopSG PDF terminal classification, v2 search repair/app cutover, and final administrator QA.
...
**Execution drift — 2026-09-16 00:55Z:** the governed database lane closed every issue this workstream was waiting on: #2792, #2860, #2937 and #2945 are closed with live-verified production evidence under the now-closed marker #2927. Independent read-only production probes as role `authenticated` with a real PopSG-entitled identity and the normal 8 s ceiling confirm the reconciliation repair but qualify the search repair: every filtered or queried `search_style_guide_library_v2` call returns in 0.01–0.75 s, while the **unfiltered default browse** runs 3.4–6.6 s in `files` mode and 6.4–7.3 s in `guides` mode, with one observed SQLSTATE 57014 at the ceiling. #2860's live proof used `files` mode, no query, `p_limit=>1`, so it does not cover the app's real default view. The v2 app cutover is therefore held and the gap was filed as shared-db #3009 and routed to active marker #3004; no shared-db change was authored, no timeout raised, and no app-side workaround adopted. Step 5 is now complete: administrator-console acceptance passed on build `f341443` with zero 4xx/5xx and zero console errors across all four Settings tabs, evidenced in `verification/popsg-readiness/admin-console-qa-2026-09-15.md`; the handoff's sole owner-blocking item is cleared because an approved protected administrator test identity already exists. The PopDAM PDF remainder of 2 is explained and is not a defect: that queue completed on 2026-09-14 and the two outstanding items are licensing PDFs that arrived afterwards, so no claim repair is pending; `count_pdf_backfill_remaining()` still takes 6.1 s and should be watched. PopSG PDF extraction remains at 0 of 4,318 because it requires the Windows render agent, which has been offline since 2026-09-14. Bridge `1.16.12` adoption is unchanged and still needs one supported Container Manager project refresh. Phases D–E were re-read; the remaining gates are v2 default-view performance, Windows-agent availability for both PDF queues, bridge adoption, then final acceptance and closure.

**Execution drift — 2026-09-16 01:35Z:** the owner brought the Windows render agent (`edge-alien`) back online; it heartbeats healthy on `0.16.3.161` with a healthy NAS preflight. The PopDAM PDF queue was resumed through the normal authenticated admin route and **failed on every heartbeat**: the agent log reports `agent-api claim-pdf-backfill-batch returned 500: claim_pdf_backfill_batch failed: canceling statement due to statement timeout`. Nothing was claimed and no rows moved. This proves shared-db #2792's closure is wider than its evidence — its live proof exercised `reconcile_stale_sg_files_batch` only, which is genuinely repaired at 93 ms, while the PDF claim path in the same issue was never measured and is still broken in production. Because PopSG extraction is serialized behind PopDAM completion in `supabase/functions/agent-api/index.ts` (`handleClaimStyleGuidePdfText`, ~line 3331), this blocks all 4,318 PopSG PDFs, not just the two leftover PopDAM files. The PopDAM job was **paused again** so it stops issuing a failing 8 s query every 30 seconds against production; it needs resuming once the claim is repaired. Evidence and the agent log line were added to shared-db #3009 and escalated to marker #3004. No timeout was raised, no app-side workaround added, and nothing was claimed or written. Separately, the owner clarified that the Synology containers were deployed from the command line and are **not** Container Manager projects, and directed that SSH with the `916-alien` key be used. SSH works as `ahazan` on both NASes, but `/var/run/docker.sock` is `root:root` 0660 and `sudo` requires a password on both hosts: a file named `ahazan-docker.sudoers-disabled` sits in `/volume1/docker/popdam` on each, dated 2026-09-03, so the passwordless docker sudo that used to exist was deliberately removed after the bridge incident. The live bridge is on `edgesynology1`, and its on-NAS `docker-compose.yml` is missing both the `POPDAM_COMPOSE_PATH` environment entry and the `/volume1/docker/popdam:/app/compose:ro` mount that repo commit `fc7bfa91` added, which is exactly why the self-updater stops. A re-runnable, backup-taking, rollback-preserving script was staged at `/volume1/docker/popdam/apply-bridge-1.16.12.sh` on `edgesynology1`; it patches the compose file in place, pulls the published image and recreates the Compose-owned service, and needs one `sudo` run. The NAS itself was not modified. Phases D–E were re-read; gates are unchanged.

**Execution drift — 2026-09-16 11:00Z:** **bridge `1.16.12` is adopted and verified in production.** The owner ran the staged script on `edgesynology1` with `sudo`. The bridge reports version `1.16.12`, build `edcd8341bb8e3b054a3ff55601f4c61aeb4f39d9`, state running, restart count 0, `com.docker.compose.project.working_dir=/volume1/docker/popdam`, the same agent id as before (no re-pair, no duplicate registration), `paired=true`, continuous heartbeats, no `last_error`, and a healthy NAS mount with readable scan roots. The compose backup and previous image id are retained for rollback and no NAS content was written. Evidence: `verification/popsg-readiness/bridge-1.16.12-adoption-2026-09-16.md`. Two corrections to earlier notes are recorded there: the live bridge is on `edgesynology1`, not edge2, and these containers are not Container Manager projects, so the long-standing "refresh the Container Manager project" instruction was never the right one. A benign startup warning notes the image runs Node 20.20.2 while the AWS SDK will require Node 22 for releases after early 2027; not urgent, but worth scheduling.

**However, the ordinary nightly crawl of 2026-09-15 22:00 FAILED in production**, which breaks the three-clean-night proof chain and restarts it from zero. Ingestion and reconciliation were clean — 217,047 files found and accepted, 0 deactivated, `stale_remaining=0`, `guard_state=ok` — and only the final aggregate step failed, with `Matview refresh failed: canceling statement due to statement timeout`. The run is recorded `status=failed`, `lifecycle_state=refreshing`, `refresh_completed_at` null, so `style_guide_file_groups` and `style_guide_folders` are stale and the Guides grid and folder tree read from them. `refresh_style_guide_matviews` performs two `refresh materialized view concurrently` operations over the 217k-row base plus a search-document batch inside a single statement under the 8 s ceiling; it passed 11–14 September and failed on the 15th, consistent with the corpus crossing the line rather than a one-off. The caller's only lever, `p_search_batch_size`, does not touch the dominant cost, so there is no honest application-side fix and none was attempted. Filed as shared-db #3023 and escalated to marker #3004. This is now the third function in the same statement-ceiling family alongside #3009's `search_style_guide_library_v2` and `claim_pdf_backfill_batch`. Phases D–E were re-read; the crawl-proof gate has reset and no manual crawl will be used to satisfy it.

**Execution drift — 2026-09-18 18:00Z:** shared-db moved from `u2giants/shared-db` to `popcre/shared-db` (old URLs redirect; `gh search` on the old name fails). #3009 closed 2026-09-17 after a governed snapshot-backed repair and a PopSG live proof; re-measured here, the unfiltered default browse now returns in 17–223 ms in both `files` and `guides` modes, so step 8's app cutover is unblocked. #3023 closed 2026-09-18 17:50Z with a queue-driven matview refresh; the Sept 15, 16 and 17 crawls all failed at that step, so the crawl proof restarts from the next ordinary night. `claim_pdf_backfill_batch(10)` still fails at 8.007 s cold — neither #2792 nor #3009 covered it — so it was re-filed as #3282 and routed to active marker #3275. The PopDAM PDF job stays paused. No shared-db change was authored from this repo.

**Execution drift — 2026-09-15 11:16Z:** closed marker #2925 was superseded by the sole active shared-db orchestrator marker #2927. The bounded reconciliation/PDF performance issue #2792 is under that lane (merged PR #2933, with its required historical-ledger restoration still claimed in #2937); no production acceptance of the repaired functions was asserted in this session. The default-v2 search successor #2860 was dispatched to claim #2945 under the same marker. No competing orchestrator, structural authoring, production write, crawl, or backfill was started here. Phases D–E were re-read: wait for the governed production evidence, then drain the existing PopDAM PDF work before PopSG extraction, cut the app over to the governed v2 search contract, obtain administrator-console QA, and complete final acceptance.
