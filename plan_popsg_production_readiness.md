...
| 3. Make crawl completion truthful and restart-safe | 🟨 in progress | Truthful failure behavior is deployed, but ordinary September 7–9 runs timed out during bounded reconciliation and September 10 stopped during its first ingest batch; all changed zero active rows. Smaller app batches are prepared and structural successor shared-db #2792 is queued; first clean ordinary-run proof remains pending |
...
| 6. Finish and classify preview coverage | 🟨 in progress | Shared-db #2509 is in production, but a fresh service-role aggregate call still hit the ordinary statement ceiling; signed-in application recheck and post-reconciliation retries remain pending |
| 7. Finish searchable PDF/source coverage | 🟨 in progress | Shared-db #2507/v2 terminal outcome contract is in production; app/Windows v2 wiring is prepared and tested. PopDAM reports 382 remaining, 23,176 processed, and no top-level failures; PopSG execution remains serialized behind its completion |
| 8. Build comprehensive PopSG search and filters | 🟨 in progress | Current production has 228,029 active rows/search documents and the v2 migrations; preliminary Files-mode QA exists. GitHub still reports #2506 open with active production-performance work, so final guide/filter acceptance remains pending |
