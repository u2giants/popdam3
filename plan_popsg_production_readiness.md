...
| 3. Make crawl completion truthful and restart-safe | 🟨 in progress | Governed contract plus app orchestration deployed; bridge v1.16.11 / `6ca879e7`; accepted-count repair `24013e55`; bounded full-corpus search-refresh continuation `9836aa9e`; the first two post-contract ordinary runs safely stopped with zero deactivations; first clean ordinary-run proof pending |
...
| 6. Finish and classify preview coverage | 🟨 in progress | Preliminary classification artifact exists; current aggregate is 220,290 previewed, 7,701 classified errors and 1 unexplained/no-status row, but signed-in Settings exposed `get_sg_preview_stats()` timeouts routed as shared-db #2509; post-reconciliation retries pending |
| 7. Finish searchable PDF/source coverage | 🟨 in progress | Serialized Windows release `0.16.3.159` is live; the single PopDAM backlog is landing durable rows; separate PopSG PDF coverage is now visible in signed-in production via `16cda85a`; PopSG execution remains gated on PopDAM completion and shared-db #2507 |
| 8. Build comprehensive PopSG search and filters | 🟨 in progress | v1 search documents cover all 227,992 currently active rows; preliminary signed-in Files-mode QA is recorded in `verification/popsg-readiness/library-qa-2026-09-07.md`; missing guide-mode pagination and launch filters remain routed as #2506 |
