...
- Description contract: `ai_description` is not a creative caption; it is a concise, search-friendly sentence for designers/salespeople looking for reusable licensed product assets. `scene_description` is a literal visual sentence. The canonical wording in `supabase/functions/_shared/tag-asset-contract.js` mentions common PopDAM asset kinds including source art, product photos, renders/mockups, professional/lifestyle photography, close-ups/details, tech packs, packaging art, stickers, and J-cards. Do not expand this into a long taxonomy in the prompt without a token-cost reason.
