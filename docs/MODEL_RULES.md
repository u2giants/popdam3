...
- Uses OpenRouter through the worker's `OPENROUTER_API_KEY` for normal model IDs. Selecting `meta-direct/muse-spark-1.2-contributor` uses Meta Model API directly through the worker's `META_API_KEY`; the Contributor tier permits Meta to use submitted thumbnails, prompts, and outputs for model training. The worker no longer falls back to `GOOGLE_AI_API_KEY` (removed 2026-07-14 — a Google key cannot authenticate against openrouter.ai). `GOOGLE_AI_API_KEY` is still live, but only for the on-prem agents' direct-Google PDF text extraction (see PDF Text Extraction below and `docs/KNOWN_QUIRKS.md` #63).
