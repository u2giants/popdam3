...
- When two or more approved Customer, Licensor, Designer, or Factory candidates remain, the admin API uses the existing server-side `OPENROUTER_API_KEY` to ask the pinned TypeSafe Jev decision model to assess only those candidates. A sufficiently confident answer adds a `Jev suggests` marker without moving the approval buttons: it cannot add candidates, create records, or save a match. `no_match` or a low-confidence answer leaves the choices unmarked. If Jev or its privacy-constrained OpenRouter route is unavailable, the UI says so and shows the existing deterministic choices.
