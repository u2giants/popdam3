...
These are OpenRouter model IDs. If not set, the worker falls back to `DEFAULT_VISION_MODEL = "google/gemini-2.0-flash-001"`. Configure via Settings → AI Models in the admin UI — the dropdowns are populated live from your OpenRouter account's guardrailed `/api/v1/models/user` list. Unavailable placeholder aliases with negative pricing are filtered out. Vision Bake-Off additionally requires image input and tool support because the worker uses required structured tool output.
